<?php

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

//Route::get('/', function () {
  //  return view('welcome');
//});
Route::get('/','Controller@welcome');


Auth::routes();

Route::get('/home', 'HomeController@index');

Route::get('admin/backhome', 'AdminController@index');
Route::get('admin', 'Admin\LoginController@showLoginForm')->name('admin.login');
Route::post('admin', 'Admin\LoginController@login');
Route::post('admin-password/email', 'Admin\ForgotPasswordController@sendResetLinkEmail')->name('admin.password.email');
Route::get('admin-password/reset', 'Admin\ForgotPasswordController@showLinkRequestForm')->name('admin.password.request');
Route::post('admin-password/reset', 'Admin\ResetPasswordController@reset');
Route::get('admin-password/email/{token}', 'Admin\ResetPasswordController@showRestForm')->name('admin.password.reset');

Route::get('/backhome', 'BackhomeController@index');

Route::get('/activity8', 'HomeController@activity8');

Route::get('/activity8', 'BackhomeController@activity8');

//Route::get('/activity8', 'HomeController@activity8');

//Route::get('/newprogram', 'HomeController@newprogram');

Route::get('/newprogram',function (){
  return view('newprogram');
});

// Route::post('/insert','Controller@insert');

Route::post('/insert','BackhomeController@insert');

Route::get('/problem',function(){
    return view('problem');
});

//Route::get('/backhome',function(){
 // return view('backhome');
//});

Route::get('/clause',function(){
  return view('clause');
});
Route::get('/news',function(){
  return view('news');
});

Route::get('/order', 'HomeController@personalorder');